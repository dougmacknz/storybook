      ...
      return (
        <Fragment>
          <Head
            {...node}
            depth={depth}
            isExpanded={expanded.indexOf(node.id) !== -1}
            isSelected={selected[node.id]}
          />
          {children && expanded.indexOf(node.id) !== -1 ? (
...
const calculateTreeState = memoize(50)(
  ({ dataset, selectedId }, { lastSelectedId, unfilteredExpanded }) => {
    ...
    const newExpanded = Object.keys(dataset).reduce(
      (acc, key) =>
        selectedAncestorIds.includes(key) || unfilteredExpanded.indexOf(key) !== -1
          ? [...acc, key]
          : [...acc],
      []
    );
  ...
  events = {
    onClick: (e, item) => {
      this.setState(
        updateExpanded(expanded =>
          expanded.indexOf(item.id) === -1 ? [...expanded, item.id] : [...expanded]
        )
      );
    },
    onFilter: inputFilter => {
      ...
      this.setState({
        filter,
        filteredExpanded:
          !!filter && Object.keys(filteredDataset).reduce((acc, key) => [...acc, key], []),
      });
    },
    onKeyUp: (e, item) => {
      ...
      if (action === 'RIGHT') {
        ...
        if (!filteredDataset[item.id].children || expanded.indexOf(item.id) !== -1) {
          if (next) {
            try {
        ...
        this.setState(updateExpanded(currExpanded => [...currExpanded, item.id]));
      }
      if (action === 'LEFT') {
